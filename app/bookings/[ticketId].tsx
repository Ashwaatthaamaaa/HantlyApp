// File: app/bookings/[ticketId].tsx (Corrected OTP Copied Alert)
...
import Modal from 'react-native-modal'; // Using react-native-modal for OTP modals
import { BASE_URL } from '@/constants/Api';
import { useHeaderHeight } from '@react-navigation/elements'; // For KeyboardAvoidingView offset
...
export default function BookingDetailScreen() {
  ...
  const [isOtpModalVisible, setOtpModalVisible] = useState(false); // For closing OTP
  const [enteredOtp, setEnteredOtp] = useState('');
  const [otpError, setOtpError] = useState('');
  const [isAcceptOtpModalVisible, setIsAcceptOtpModalVisible] = useState(false); // For acceptance OTP
  ...
  const fetchBookingDetails = useCallback(async () => {
      ...
      console.log(`BookingDetailScreen: Fetching details for ticketId: ${ticketId}. isLoading was: ${isLoading}`);
      setIsLoading(true); setError(null);
      const url = `${BASE_URL}/api/IssueTicket/GetTicket?TicketId=${ticketId}`;
      console.log("BookingDetailScreen: Fetching details from:", url);

      try {
          const response = await fetch(url);
          if (!response.ok) { 
              const errorText = await response.text(); 
              console.error(`BookingDetailScreen: HTTP error ${response.status} fetching details: ${errorText}`); 
              throw new Error(t('failedfetchdetails', { status: response.status, errorText: errorText || 'Server error' })); 
          }
          const data: BookingDetail = await response.json();
          console.log("BookingDetailScreen: Details fetched successfully.", data);
          setBookingData(data);
          setSelectedRating(data.reviewStarRating || 0); // Initialize with existing rating
          setReviewCommentText(data.reviewComment || '');   // Initialize with existing comment
          setIsSubmittingReview(false); // Reset review submission state
      } catch (err: any) { 
          console.error("BookingDetailScreen: Error fetching booking details:", err); 
          setError(t('failedloadbookingdetails', { message: err?.message || 'Unknown error' }));
          setBookingData(null); 
      }
      finally { 
          console.log(`BookingDetailScreen: Fetch details finished for ticketId: ${ticketId}. Setting isLoading to false.`);
          setIsLoading(false); 
      }
   ...
   const fetchChatList = useCallback(async () => {
       ...
       console.log("BookingDetailScreen: Fetching chat list from:", url);
       try {
           const response = await fetch(url);
           if (!response.ok) { 
               const errorText = await response.text(); 
               throw new Error(`Failed to fetch chat list (${response.status}): ${errorText}`); 
           }
           ...
           if (Array.isArray(data)) {
               ...
               setInterestedPartners(validPartners); 
               console.log(`BookingDetailScreen: Fetched ${validPartners.length} interested partners.`);
           } else { 
               console.warn("BookingDetailScreen: GetChatList did not return an array:", data); 
               setInterestedPartners([]);
               // Consider not throwing error if backend can return non-array for no partners.
               // throw new Error("Unexpected response format from GetChatList."); 
           }
       } catch (err: any) { 
           console.error("BookingDetailScreen: Error fetching chat list:", err);
           setChatListError(t('errorloadingpartners', { error: err?.message || 'Unknown error' })); 
           setInterestedPartners([]); 
       }
   ...
   const updateStatus = useCallback(async (
       ...
       if (requiresOtpValidation && (otpApiValue === null || isNaN(otpApiValue) || otpApiValue <= 0)) {
            ...
            Alert.alert(t('error'), errorMsg || t('invalidotp'));
            return false;
       }
       ...
       console.log(`BookingDetailScreen: Updating status API Call: URL=${url}, Body=${body}`);
       try {
           const response = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json', 'accept': 'text/plain' }, body: body });
           const responseText = await response.text();
           console.log(`BookingDetailScreen: Update Status Response (${newStatus}): Status=${response.status}, Body=${responseText}`);
           if (!response.ok) {
           ...
           try { const result = JSON.parse(responseText); successMessage = result?.statusMessage || successMessage; } catch(e) { /* Ignore */ }
           Alert.alert(t('success'), successMessage);
           await fetchBookingDetails(); 
           return true;
       } catch (err: any) {
           console.error(`BookingDetailScreen: Failed to update status to ${newStatus}:`, err);
   ...
   const handleCallCustomer = useCallback(() => { handleCall(bookingData?.userMobileNumber); }, [bookingData?.userMobileNumber, handleCall]);
    ...
    // --- MODIFIED OTP Copy Functions ---
    const handleCopyOtp = useCallback(async () => { // For Closing OTP
        const otp = bookingData?.closingOTP;
        if (otp != null) {
            try {
                await Clipboard.setStringAsync(otp.toString());
                Alert.alert(t('otpcopied'), `${t('otp_value_is')} ${otp.toString()}`); // Show OTP in alert
            } catch (e) {
                Alert.alert(t('error'), t('couldnotcopyotp'));
            }
        } else {
            Alert.alert(t('nootp'), t('otpnotavailable'));
        }
    }, [bookingData?.closingOTP]);

    const handleCopyAcceptOtp = useCallback(async () => { // For Acceptance OTP
        const otp = bookingData?.acceptedOTP;
        if (otp != null) {
            try {
                await Clipboard.setStringAsync(otp.toString());
                Alert.alert(t('otpcopied'), `${t('otp_value_is')} ${otp.toString()}`); // Show OTP in alert
    ...
    }, [bookingData?.acceptedOTP]);
    // --- END MODIFIED OTP Copy Functions ---
  ...
  useFocusEffect(useCallback(() => { 
      console.log(`BookingDetailScreen: Focus effect for ticketId: ${ticketId}`);
      fetchBookingDetails(); 
    }, [fetchBookingDetails]) // fetchBookingDetails has ticketId in its own deps
  );

  useEffect(() => {
     if (session?.type === 'user' && bookingData && bookingData.status?.toLowerCase() === 'created') {
         console.log("BookingDetailScreen: User view, status 'created', fetching chat list.");
         fetchChatList();
     } else {
         if (interestedPartners.length > 0 || isLoadingChatList || chatListError) {
           console.log("BookingDetailScreen: Clearing chat list state (not user or not created status).");
           setInterestedPartners([]);
  ...
  const partnerCanInitiateChat = isPartner && isCreated && !!bookingData.userId; // Allows partner to message user after user creates job
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['left', 'right', 'bottom']}>
       <Stack.Screen
        options={{
          title: bookingData.reportingDescription?.substring(0,25) + (bookingData.reportingDescription && bookingData.reportingDescription.length > 25 ? "..." : "") || t('bookingdetails'),
          headerBackTitle: '',
      ...
      <KeyboardAvoidingView 
        ...
        style={{flex:1}} 
        keyboardVerticalOffset={headerHeight + (Platform.OS === 'ios' ? 10 : 0) } // Adjust offset as needed
      >
            ...
            <View style={styles.statusHeader}>
                ...
                {userCanSeeAcceptOtp && (
                    <TouchableOpacity style={styles.otpContainer} onPress={handleCopyAcceptOtp}>
                        <Text style={styles.otpText}>{`${t('otp')}: ${bookingData.acceptedOTP}`}</Text>
                ...
                {userCanSeeOtp && ( // Closing OTP
                    <TouchableOpacity style={styles.otpContainer} onPress={handleCopyOtp}>
                        <Text style={styles.otpText}>{`${t('otp')}: ${bookingData.closingOTP }`}</Text>
                        <Ionicons name="copy-outline" size={16} color={COLORS.textSecondary} style={{ marginLeft: 5 }}/>
                    </TouchableOpacity>
                )}
            </View>
...
const styles = StyleSheet.create({
    ...
    errorTextSmall: { color: COLORS.error, fontSize: 12, textAlign: 'center', paddingHorizontal: 15, paddingVertical: 5, backgroundColor: '#FFEEEE', borderRadius: 4, marginBottom: 10},
    ...
    phoneLink: { color: COLORS.statusCreated }, // Removed textDecorationLine for native
    ...
    actionRowDisabled: { backgroundColor: COLORS.actionRowDisabled, opacity: 0.6 },
    disabledText: { color: COLORS.textSecondary },
    imageViewerFooter: { height: 50, backgroundColor: 'rgba(0, 0, 0, 0.7)', justifyContent: 'center', alignItems: 'center', }, // Darker footer
    imageViewerFooterText: { color: 'white', fontSize: 16, },
    otpModalContainer: { justifyContent: 'center', alignItems: 'center', margin: 0 }, // react-native-modal specific
    ...
    otpInput: { borderWidth: 1, borderColor: COLORS.borderColor, backgroundColor: '#f9f9f9', paddingVertical: 12, paddingHorizontal: 20, borderRadius: 8, width: '80%', textAlign: 'center', fontSize: 28, marginBottom: 15, letterSpacing: 15, fontWeight: 'bold', },
    ...
    otpModalButton: { flex: 1, paddingVertical: 12, borderRadius: 8, alignItems: 'center', marginHorizontal: 5, minHeight: 46, justifyContent: 'center'},
    ...
    proofThumbnailsContainer: { marginTop: 5, marginBottom: 10, paddingLeft: 2 },
    ...
    reviewInputSection: { borderTopWidth: 1, borderTopColor: COLORS.borderColor, marginTop: 10, paddingTop: 15, backgroundColor: '#fdfdfd', paddingHorizontal: 15, marginHorizontal: -20, paddingBottom: 15, borderRadius: 5 },
    ...
    starRatingDisplay: { flexDirection: 'row', alignItems: 'center', marginBottom: 10, marginTop: 5 },
...
// Add/ensure these translation keys exist in en.ts and sv.ts:
// t('otp_value_is') // e.g., "OTP value copied:" or "OTP:"