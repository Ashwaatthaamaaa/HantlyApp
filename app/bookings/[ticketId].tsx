...
export default function BookingDetailScreen() {
  ...
  useEffect(() => {
      if (session?.type === 'user' && bookingData && bookingData.status?.toLowerCase() === 'created') { fetchChatList(); }
      else { if (interestedPartners.length > 0 || isLoadingChatList || chatListError) { setInterestedPartners([]); setIsLoadingChatList(false); setChatListError(null); } }
  }, [bookingData, session, fetchChatList, interestedPartners.length, isLoadingChatList, chatListError]);
  ...
  const updateStatus = useCallback(async (newStatus: "Accepted" | "Inprogress" | "Completed", otp: number | null = 0): Promise<boolean> => {
      ...
      try {
          ...
          Alert.alert(t('success'), successMessage); await fetchBookingDetails(); return true;
