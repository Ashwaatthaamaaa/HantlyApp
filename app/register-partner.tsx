...
export default function RegisterPartnerScreen() {
  ...
  const municipalityPlaceholder = selectedCountyIds.length === 0 ? 'Select County First' : isLoadingMunicipalities ? 'Loading...' : municipalityError ? 'Error Loading' : municipalities.length === 0 ? 'No Municipalities Found' : 'Select Municipality';
   ...
   // --- Partner Sign Up Handler (Added Is24X7) ---
   const handleSignUp = async () => {
    // --- Validation (remains same) ---
    // ... (keep existing validation logic) ...
    const requiredFields = [ { value: companyName, name: "Company Name" }, { value: companyLogo, name: "Company Logo" }, { value: description, name: "Company Description" }, { value: phone, name: "Phone Number" }, { value: email, name: "Email" }, { value: password, name: "Password" }, ]; const missingFields = requiredFields.filter(field => !field.value); const trimmedEmail = email.trim(); const trimmedPassword = password; if (missingFields.length > 0) { Alert.alert('Missing Info', `Provide: ${missingFields.map(f => f.name).join(', ')}`); return; } if (selectedCountyIds.length === 0) { Alert.alert('Missing Info', 'Select County.'); return; } if (selectedMunicipalityIds.length === 0) { Alert.alert('Missing Info', 'Select Municipality.'); return; } if (selectedServiceIds.length === 0) { Alert.alert('Missing Info', 'Select Service Category.'); return; } if (trimmedPassword.length < 8) { Alert.alert('Password Too Short', 'Min 8 chars.'); return; } if (!/\S+@\S+\.\S+/.test(trimmedEmail)) { Alert.alert('Invalid Email'); return; }
    ...
    // ADDED: Append the Is24X7 value (as string)
    ...
    console.log('FormData Keys:', [...(formData as any)._parts.map((part: any[]) => part[0])].join(', ')); // Log keys
     console.log('Is24X7 value being sent:', is24x7Enabled.toString()); // Log 24x7 value
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        ...
        {/* Phone, Email, Password (remain same) */}
...
const styles = StyleSheet.create({
  ...
  switchContainer: {
      ...
      alignItems: 'center',
      marginBottom: 20, // Add margin below switch
      paddingHorizontal: 5, // Optional padding
  },
  inputLabel: { // Style for the label next to the switch
      fontSize: 16,
